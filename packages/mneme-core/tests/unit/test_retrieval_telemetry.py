...
Covers:
- RetrievalEvent dataclass roundtrip (fields present, query_hash is sha256 not raw)
...
def test_query_hash_is_sha256_not_raw(tmp_path: Path) -> None:
    """emit_retrieval_event stores sha256(normalized_query), never raw query text."""
    vault = _make_vault(tmp_path)
    raw_query = "cognitive load research"
    expected_hash = hashlib.sha256(raw_query.encode()).hexdigest()
    event = RetrievalEvent(
...
def test_retrieve_wires_telemetry(_indexed_vault: tuple[VaultConfig, Path]) -> None:
    ...
    for key in ("query_hash", "backends", "per_backend_hits", "per_backend_latency_ms",
                 "fused_count", "timestamp_iso"):
