...
def test_invalid_persisted_scope_fails_closed() -> None:
    with pytest.raises(ValueError):
        persisted_scope("*")
    assert not scope_matches("bad*scope", "*")  # corrupted metadata stays hidden
    assert not scope_matches("bad*scope", "clinical")

from mneme_core.scope import (  # noqa: E402
    DocumentScopeError,
...
def test_markdown_scope_malformed_frontmatter_fails_closed() -> None:
