...
describe("VaultConfig.resolve resolution order", () => {
	...
	it("throws VaultNotFoundError with remediation hint when nothing resolves", () => {
		const home = freshTmp("orphan-home");
		const orphan = join(home, "orphan");
		mkdirSync(orphan);
		expect(() =>
			VaultConfig.resolve({
				cwd: orphan,
	...
	it("error message mentions MNEME_VAULT and .mneme/ marker", () => {
		const home = freshTmp("orphan-msg-home");
		const orphan = join(home, "orphan");
		mkdirSync(orphan);
		try {
			VaultConfig.resolve({
...
describe("VaultConfig TOML config", () => {
	it("reads vault key before marker walk when env and explicit are empty", () => {
		const home = freshTmp("toml-home");
		const configuredVault = freshTmp("toml-configured");
		const markerVault = freshTmp("toml-marker");
		mkdirSync(join(home, ".mneme"));
		mkdirSync(join(markerVault, ".mneme"));
		writeFileSync(
			join(home, ".mneme", "config.toml"),
			`vault = "${configuredVault.replaceAll("\\", "/")}"\n`,
		);
		const cfg = VaultConfig.resolve({
			cwd: markerVault,
			home,
		...
		expect(cfg.root).toBe(configuredVault);
	});
});
...
describe("VaultConfig.defaultScope resolution (M5)", () => {
	...
	it("falls closed to default for wildcard or malformed configured scope", () => {
		const cfg = VaultConfig.fromPath(freshTmp("scope-invalid"));
