...
function removeScopeColumn(dbPath: string): void {
	const db = new Database(dbPath);
	try {
		db.prepare("DROP INDEX idx_documents_scope").run();
		db.prepare("ALTER TABLE documents DROP COLUMN scope").run();
	} finally {
		db.close();
	}
}
...
describe("primeTool runtime", () => {
	...
	it("fails closed for a concrete scope on a legacy pre-scope index", () => {
		const { vault } = makeTempVault("prime-legacy-scope", defaultDocs());
		removeScopeColumn(vault.fts5Db);
		...
		if (!res.ok) {
			...
			expect(res.error.message).toContain("index rebuild");
		}
	});

	it("preserves explicit cross-scope prime on a legacy pre-scope index", () => {
		const docs: TestDoc[] = [
			{
				path: "daily/alpha.md",
				title: "Alpha Rank Session",
				titleNormalized: "alpha rank session",
				contentRaw: "Shared rank context from alpha.",
				bodyText: "Shared rank context from alpha.",
				contentNormalized: "shared rank context from alpha",
				mtime: 1_716_000_000,
			...
			{
				path: "daily/beta.md",
				title: "Beta Rank Session",
				titleNormalized: "beta rank session",
				contentRaw: "Shared rank context from beta.",
				bodyText: "Shared rank context from beta.",
				contentNormalized: "shared rank context from beta",
				mtime: 1_717_000_000,
				frontmatterType: "session",
				scope: "beta",
		...
		const { vault, rootDir } = makeTempVault("prime-legacy-star", docs);
		writeBody(rootDir, "daily/alpha.md", docs[0]?.bodyText ?? "");
		writeBody(rootDir, "daily/beta.md", docs[1]?.bodyText ?? "");
		removeScopeColumn(vault.fts5Db);

		const res = primeTool(
			PrimeInputSchema.parse({
				task_description: "rank session",
		...
		if (res.ok) {
			expect(
				[...new Set(res.data.sources.map((source) => source.path))].sort(),
			).toEqual(["daily/alpha.md", "daily/beta.md"]);
		}
	});

	it("uses the valid Turkish dual index for I, İ, ı, and i queries", () => {
		const title = "İzmir Limanı";
		const body = "İzmir limanı bugün açıktır.";
		const docs: TestDoc[] = [
			{
				path: "reference/izmir.md",
				title,
				titleNormalized: normalizeTr(title),
				titleAsciiNormalized: normalizeTrAsciiFold(title),
				contentRaw: body,
				bodyText: body,
				contentNormalized: normalizeTr(body),
				contentAsciiNormalized: normalizeTrAsciiFold(body),
				mtime: 1_717_100_000,
				frontmatterType: "reference",
				contentHash: "izmir-hash",
			},
		];
		const { vault } = makeTempVault("prime-tr-dual", docs);

		for (const query of ["IZMIR", "İZMİR", "ızmır", "izmir"]) {
			const res = primeTool(
				PrimeInputSchema.parse({
			...
			expect(res.ok).toBe(true);
			if (res.ok) {
				expect(res.data.sources).toContainEqual({
					path: "reference/izmir.md",
					kind: "topic",
				});
			}
		...
		const docs = [
			{
				...
				bodyText: body,
				contentNormalized:
					"rank fusion with frontmatter algorithm body content here.",
				mtime: 1_717_100_000,
				frontmatterType: "reference",
			},
		];
		...
		const docs: TestDoc[] = [
			{
				...
				contentRaw:
					"Public prefix. <private>hidden value</private> Public suffix.",
				bodyText:
					"Public prefix. <private>hidden value</private> Public suffix.",
				contentNormalized:
					"secret reference public prefix hidden value public suffix",
				mtime: 1_717_200_000,
				frontmatterType: "reference",
			},
		];
...
describe("primeTool key_points → keypoints format bullets", () => {
	it("emits bullet lines when key_points are stored in DB and doc is re-injected", () => {
		...
		db.prepare(`UPDATE documents SET key_points = ? WHERE path = ?`).run(
