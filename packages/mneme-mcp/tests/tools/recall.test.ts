...
describe("RecallInputSchema", () => {
  ...
  it("rejects malformed date_from", () => {
    expect(() => RecallInputSchema.parse({ date_from: "2026/05/18" })).toThrow();
  });
...
describe("recallTool runtime", () => {
  ...
  it("filters by session_id", () => {
    ...
    const res = recallTool(
      RecallInputSchema.parse({ session_id: "s-2026-05-18", include_body: false }),
      vault,
    );
    expect(res.ok).toBe(true);
    if (res.ok) {
      for (const e of res.data.entries) expect(e.session_id).toBe("s-2026-05-18");
    }
  });
