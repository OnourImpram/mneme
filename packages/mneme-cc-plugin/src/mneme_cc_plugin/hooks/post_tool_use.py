...
def _compress_bash_payload(event: dict[str, Any]) -> None:
    ...
    Which fields qualify is decided by ``iter_compressible_outputs`` in
    mneme_core, the single definition of the compression contract, so
    ``mneme audit`` measures exactly the payloads this hook rewrites.
    """
    resp = event.get("tool_response")
    if not isinstance(resp, dict):
        return
    opts = ShellCompressOpts()
    for key, value in list(iter_compressible_outputs(event)):
