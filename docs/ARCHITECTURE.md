...
**Consequences**: Codex users get the same nine MCP tools, the same two skills, and four of the six registered Claude Code hook events (SessionStart, PostToolUse, Stop, PreCompact). SessionEnd folds into Stop because Codex has no SessionEnd event. UserPromptSubmit has no Codex mapping. The Claude Code experience is unchanged and stays full-fidelity. Trade-off: two plugin manifests and two marketplace files to keep in sync, and the PostToolUse Bash-output compression is tuned to Claude Code tool names so Codex capture is generic until a Codex tool-name variant lands. The shared core means a retrieval or vault improvement benefits both clients at once.
